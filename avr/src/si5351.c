...
static uint32_t plla_freq = 0;
static uint32_t pllb_freq = 0;
 ...
 */
static uint32_t XtalFreq = 30000000;

void si5351_init(uint32_t XtalFrequency) {
//	i2c_init();

	/* Set crystal load capacitance */
	i2c_write1Reg(SI5351_BUS_BASE_ADDR, SI5351_CRYSTAL_LOAD, SI5351_CRYSTAL_LOAD_8PF); //si5351_write(SI5351_CRYSTAL_LOAD, SI5351_CRYSTAL_LOAD_8PF);

	/* Set the Xtal Frequency*/
	//ref_correction = eeprom_read_dword(&ee_ref_correction);
	XtalFreq = XtalFrequency;
}
...
void si5351_set_freq(uint32_t freq, uint32_t pll_freq, enum si5351_clock clk)
{
	...
	if( pll_calc(pll_freq, &pll_reg) == 0)
	{
		/* Registers 26-27 */
		...
		{
			i2c_writeReg(SI5351_BUS_BASE_ADDR, SI5351_PLLA_PARAMETERS, params, i+1); 	//si5351_write_bulk(SI5351_PLLA_PARAMETERS, i + 1, params);
		}
		else if(target_pll == SI5351_PLLB)
		{
			i2c_writeReg(SI5351_BUS_BASE_ADDR, SI5351_PLLB_PARAMETERS, params, i+1); 	//si5351_write_bulk(SI5351_PLLB_PARAMETERS, i + 1, params);
		}
	}
	...
	{
	case SI5351_CLK0:
		i2c_writeReg(SI5351_BUS_BASE_ADDR, SI5351_CLK0_PARAMETERS, params, i + 1); //si5351_write_bulk(SI5351_CLK0_PARAMETERS, i + 1, params);
		si5351_set_ms_source(clk, target_pll);
		break;
	case SI5351_CLK1:
		i2c_writeReg(SI5351_BUS_BASE_ADDR, SI5351_CLK1_PARAMETERS, params, i + 1); //si5351_write_bulk(SI5351_CLK1_PARAMETERS, i + 1, params);
		si5351_set_ms_source(clk, target_pll);
		break;
	case SI5351_CLK2:
		i2c_writeReg(SI5351_BUS_BASE_ADDR, SI5351_CLK2_PARAMETERS, params, i + 1); //si5351_write_bulk(SI5351_CLK2_PARAMETERS, i + 1, params);
		si5351_set_ms_source(clk, target_pll);
		break;
	case SI5351_CLK3:
		i2c_writeReg(SI5351_BUS_BASE_ADDR, SI5351_CLK3_PARAMETERS, params, i + 1); //si5351_write_bulk(SI5351_CLK3_PARAMETERS, i + 1, params);
		si5351_set_ms_source(clk, target_pll);
		break;
	case SI5351_CLK4:
		i2c_writeReg(SI5351_BUS_BASE_ADDR, SI5351_CLK4_PARAMETERS, params, i + 1); //si5351_write_bulk(SI5351_CLK4_PARAMETERS, i + 1, params);
		si5351_set_ms_source(clk, target_pll);
		break;
	case SI5351_CLK5:
		i2c_writeReg(SI5351_BUS_BASE_ADDR, SI5351_CLK5_PARAMETERS, params, i + 1); //si5351_write_bulk(SI5351_CLK5_PARAMETERS, i + 1, params);
		si5351_set_ms_source(clk, target_pll);
		break;
	case SI5351_CLK6:
		i2c_writeReg(SI5351_BUS_BASE_ADDR, SI5351_CLK6_PARAMETERS, params, i + 1); //si5351_write_bulk(SI5351_CLK6_PARAMETERS, i + 1, params);
		si5351_set_ms_source(clk, target_pll);
		break;
	case SI5351_CLK7:
		i2c_writeReg(SI5351_BUS_BASE_ADDR, SI5351_CLK7_PARAMETERS, params, i + 1); //si5351_write_bulk(SI5351_CLK7_PARAMETERS, i + 1, params);
		si5351_set_ms_source(clk, target_pll);
		break;
	}
...
void si5351_clock_enable(enum si5351_clock clk, uint8_t enable)
{
	...
	if( i2c_read1Reg( SI5351_BUS_BASE_ADDR, SI5351_OUTPUT_ENABLE_CTRL, &reg_val) != 0)  //if(si5351_read(SI5351_OUTPUT_ENABLE_CTRL, &reg_val) != 0)
	{
		return;
	}
	...
	i2c_write1Reg(SI5351_BUS_BASE_ADDR, SI5351_OUTPUT_ENABLE_CTRL, reg_val); //si5351_write(SI5351_OUTPUT_ENABLE_CTRL, reg_val);
}
...
void si5351_drive_strength(enum si5351_clock clk, enum si5351_drive drive)
{
	...
	if( i2c_read1Reg(SI5351_BUS_BASE_ADDR, SI5351_CLK0_CTRL + (uint8_t)clk, &reg_val) != 0) //if(si5351_read(SI5351_CLK0_CTRL + (uint8_t)clk, &reg_val) != 0)
	{
		return;
	}
	...
	i2c_write1Reg(SI5351_BUS_BASE_ADDR, SI5351_CLK0_CTRL + (uint8_t)clk, reg_val); //si5351_write(SI5351_CLK0_CTRL + (uint8_t)clk, reg_val);
}
...
uint32_t pll_calc(uint32_t freq, struct Si5351RegSet *reg) {
	uint32_t ref_freq = XtalFreq;
...
{
	i2c_writeReg(SI5351_BUS_BASE_ADDR,addr,data,bytes);
...
void si5351_update_sys_status(struct Si5351Status *status)
{
	...
	if (i2c_read1Reg(SI5351_BUS_BASE_ADDR, SI5351_DEVICE_STATUS, &reg_val) != 0)  //if(si5351_read(SI5351_DEVICE_STATUS, &reg_val) != 0)
	{
		return;
	}
...
void si5351_set_ms_source(enum si5351_clock clk, enum si5351_pll pll)
{
	...
	if (i2c_read1Reg(SI5351_BUS_BASE_ADDR, SI5351_CLK0_CTRL + (uint8_t)clk, &reg_val2) != 0) //if ( si5351_read(SI5351_CLK0_CTRL + (uint8_t)clk, &reg_val2) != 0)
	{
		return;
	}
